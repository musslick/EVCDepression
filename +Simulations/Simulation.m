classdef Simulation < handle
    ...
    properties
        ...
        saveParams                              % flag: if set to true, save simulation parameters
    ...
    methods
        ...
        function this = Simulation()
            ...
            this.nSubj = 1;
            temp.RTconst = 1;       % double: offset for impact of RT on reward rate (set to 1, if RTscale = 0)
            temp.RTscale = 1;       % double: impact of RT on reward rate (EVC): RR = Reward / (RTconst + RTscale * RT)
            this.binaryErrors = 0;
            this.wrapper = 0;
            ...
            temp.rewardFnc.params{1} = temp.RTconst;
            temp.rewardFnc.params{2} = temp.RTscale;
...
%             this.learningFnc(2).input{1} = @() this.learningFnc(2).EVCModel.getOutcomeProb(1);           % dynamic input value for learning function
            ...
            % first learning rule learns outcome values & stimulus-response
            % tendencies for a given trial based on observation (delta-rule)
            temp.learningFnc(1).params{1} = 0.5;                                          % learning rate for other state parameters (learning by exposure)
            temp.learningFnc(1).type = LearningFnc.SIMPLE_STATE_RL_UW;                     % learning function type
            temp.learningFnc(1).EVCM = temp.EVCDummy;                                   % EVCModel dummy 
            this.learningFnc(1) = LearningFnc(temp.learningFnc(1).type, temp.learningFnc(1).params, temp.learningFnc(1).EVCM);
            
            %% control parameters: default cost function

            temp.costFnc.params{1} = 4;                                                  % parameter for exponential cost function (see EVC.CCost.m for implementation details), scalar value on intensity in exponent
            temp.costFnc.params{2} = -1;                                                 % parameter for exponential cost function (see EVC.CCost.m for implementation details), additive term in exponent
...
%             temp.costFnc.type = EVCFnc.LINEAR;                                              % use exponential cost fnc
            ...
            temp.trial.outcomeValues = [this.rewardVal this.nonRewardVal];              % reward for correct outcome = 2; no reward/punishment for incorrect outcome
            ...
            temp.trial.stimRespMap   = [1 0;                                            % responding to stimulus 1 tends to produce first outcome by 100%
                                                      0 1];                                           % responding to stimulus 2 tends to produce second outcome by 100%
    ...
    methods (Access = public)
        ...
        function runSubj(this)
            ...
            for TrialIdx = 1:length(this.EVCM.State.TaskEnv.Sequence)
                ...
                if(this.EVCM.State.terminate == 1)
                    break
                end
    ...
    methods (Access = public)
        ...
        function handle = plotCtrlIntensity(this, varargin)
           ...
           for i = 1:length(trials2Plot)
              baseline = 0;
              for j = 1:nSignals
                 handle{j} = plot([trials2Plot(i) trials2Plot(i)], [baseline baseline+ctrlIntensities(j,trials2Plot(i))], ...
        ...
        function plotER(this, varargin)
           ...
           if(ER2Plot == 1)
               titleStr = 'Error Rate (expected)';
           elseif (ER2Plot == 2)
               titleStr = 'Error Rate (actual)';
           end
           title(titleStr,'FontSize',this.plotParams.titleFontSize,'FontWeight','bold'); 
        end
        ...
        function plotDifficulty(this, varargin)
           ...
           for i = 1:length(subj2Plot)
                color = this.plotParams.defaultColor;
                stimSaliency = vec2mat([this.subjData(1).Log.Trials(trials2Plot).stimSalience],length(trials2Plot),2);
