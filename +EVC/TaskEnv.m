    ...
    methods
        ...
        function this = TaskEnv(Obj, varargin)
            ...
            for (i = 1:length(this.Sequence))
                ...
                if(length(this.Sequence(i).cueID) == 1)
                    % already registered this cueID?
                    if(length(allCueIDs) >= this.Sequence(i).cueID)
                        % already registered this trial type?
                        if(~ismember(this.Sequence(i).typeID, allCueIDs(this.Sequence(i).cueID).typeIDs))
                            allCueIDs(this.Sequence(i).cueID).typeIDs = [allCueIDs(this.Sequence(i).cueID).typeIDs this.Sequence(i).typeID];
                            this.trialTypes{this.Sequence(i).typeID} = this.Sequence(i);
                        end
                    else
                        % if cueID not registered, then create new entry
                        allCueIDs(this.Sequence(i).cueID).typeIDs = this.Sequence(i).typeID;
                        this.trialTypes{this.Sequence(i).typeID} = this.Sequence(i);
                    end
                end
            end
            ...
            for (i = 1:length(this.Sequence))
                ...
                if(length(this.Sequence(i).cueID) > 1)
                    ...
                    for(j = 1:length(this.Sequence(i).cueID))
                        % figure out valid trial type IDs for this cue ID
                        if(this.Sequence(i).cueID(j) <= length(allCueIDs))
                            commonIDs = allCueIDs(this.Sequence(i).cueID(j)).typeIDs;
                            if(length(commonIDs) >= 1)
                                % add trial IDs
                                choiceIDs = [choiceIDs commonIDs];
                            else 
                    ...
                    this.Sequence(i).typeID = choiceIDs;
                    ...
                    if(length(this.Sequence(i).typeID) == 0)
                        error('Choice trial has no valid type IDs to choose from');
                    end
                    
                    % if there is only one trial ID to choose from, then
                    % make this trial a cued trial
                    if(length(this.Sequence(i).typeID) == 1)
                        this.Sequence(i) = this.trialTypes{this.Sequence(i).typeID};
                        EVC.HelperFnc.warning('A choice trial just turned into a cued trial (only one possible trial type to choose from).');
                    end
                end
            end 
        end
        ...
        % updates all trials with the same type ID
        function updateSubsequentTrials(this, typeID, newTrial)
            for i = this.CurrTrialIdx:length(this.Sequence)
                
                if(this.Sequence(i).typeID == typeID)
                    this.Sequence(i) = newTrial;
                end
        ...
        function TaskEnvironment = blockedTaskSwitchDesign(trialTypes, nTrials, varargin)
           ...
           if(length(varargin) >= 2)
              ...
              if(length(trialProps) ~= length(trialTypes))
                 error('Size of trial proportion vector doesn''t match number of given trials.'); 
              end
              gcdVal = trialProps(1);
