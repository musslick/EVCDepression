classdef EVCFnc < handle
    ...
    properties (Constant)
        ...
        paramReq = [1, ... VALUE:       f(x) = p1
                    ...
                    4, ... REWRATE_EXT:     f(x) = sum(P(outcome) * (p3 * V(outcome) + p4)/ (p1 + p2 * reactionTime(outcome)), outcomes)
                    ];   
    end
    ...
    methods (Access = public)
        ...
        function out = getVal(this, varargin)
            ...
            if(this.type == this.RANDUNIFORM)
                out = getRandomUniformOut(this);
            end
            
            if(this.type == this.REWRATE_EXT)
                out = getRewardRateExtended(this, inputVal);
            end
...
%            
%         end
        % note additive term -1 for offsetting
        function out = getExpOut(this, inputVal)
            
            out = exp(this.params{1}*inputVal+this.params{2}) - 1;
            
        end
        
        % POW: power function
        function out = getPowOut(this, inputVal)
            
            out = this.params{1} .* ((inputVal + this.params{2}).^(this.params{3}));
            
        end
        
        % RANDU: unform random value
        function out = getRandomUniformOut(this)
        ...
        function out = getRewardRate(this, inputVal)
            ...
            EVC.HelperFnc.disp(source, 'size(performance.probs)', size(performance.probs), 'size(values)', size(values), 'size(repmat(transpose(values),1,size(performance.probs,2)))', size(repmat(transpose(values),1,size(performance.probs,2)))); % DIAGNOSIS
            ...
            meanRTs = sum(performance.probs.*performance.RTs,1);
        ...
        % REWRATE_EXT: Reward Rate (Extended) ... calculate expected value of an outcome
        % divided by weighted reaction time (+ offset) for this outcome;
        % sum values over all outcomes
        % inputVal ...current state
        function out = getRewardRateExtended(this, inputVal)
            ...
            % pre-specified trial outcomes
            if(this.EVCModel.useActualState)
                values = this.params{3} * this.EVCModel.State.Actual.outcomeValues +  this.params{4};
            else
                values = this.params{3} * this.EVCModel.State.Expected.outcomeValues +  this.params{4};
            end
